...
def register_route_backend_group_documents(app):
    ...
    @app.route('/api/group_bulk_documents/upload', methods=['POST'])
    #@login_required
    #@user_required
    @enabled_required("enable_group_workspaces")
    def api_upload_group_bulk_document():
        """
        Bulk Upload one or more documents to the currently active group.
        Mirrors logic from api_user_upload_document but scoped to group context.
        """
        #user_id = get_current_user_id()
        #if not user_id:
        ...
        #user_settings = get_user_settings(user_id)
        #active_group_id = user_settings["settings"].get("activeGroupOid")
        active_group_id = request.form.get('activeGroupOid')
